...
fn blocking_write_loop(mut rx: mpsc::Receiver<ServerPacket>) {
    ...
    loop {
        if let Some(server_packet) = rx.blocking_recv() {
            ...
            // Prepare Ethernet frame and send
            // We must create a raw socket to send ethernet frames directly to the MAC address.
            // For demonstration, assuming the socket is a SOCK_DGRAM socket bound for IP, to send raw Ethernet frames normally requires SOCK_RAW
            // Since original code is using socket2::Socket of type DGRAM and domain IPV4, actual raw ethernet sending is non-trivial.
            // Instead, we can use sockaddr_ll with the MAC address for Linux raw socket send.
            // Here is an example with unsafe socket syscall if possible, otherwise we fallback to send_to IP.
            //
            ...
            unsafe {
                ...
                // Send the packet to the MAC address
